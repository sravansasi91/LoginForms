...
const fetchApiData = (action: any) => NetworkService({
  ...
  function* fetchApiRequestSaga(action: any): any {
    yield put({ type: PermanentAddressTypes.PERM_ADDRESS_LOADER, loading: true });
    const response : any = yield call(fetchApiData, action);
    if (response) {
      yield put({
        type: PermanentAddressTypes.PERM_ADDRESS_SUCCESS,
        payload: {
          data: response.data
        },
      });
      ...
      yield put({
        type: PermanentAddressTypes.PERM_ADDRESS_ERROR,
        error: {
          error: response
        }
      });
  ...
  export default function* watcherSaga() {
    yield takeLatest(PermanentAddressTypes.PERM_ADDRESS, fetchApiRequestSaga);
  }