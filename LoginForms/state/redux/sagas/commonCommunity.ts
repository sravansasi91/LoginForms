...
const fetchApiData = (action: any) => NetworkService({
  ...
  function* fetchApiRequestSaga(action: any): any {
    ...
    yield put({ type: CommonCommunityActionTypes.COMMUNITY_LOADER, loading: true });
    const response : any = yield call(fetchApiData, action);

    if (response) {
      yield put({
        type: CommonCommunityActionTypes.COMMUNITY_SUCCESS,
        payload: {
          data: response?.data
        },
      });
      ...
      yield put({
        type: CommonCommunityActionTypes.COMMUNITY_ERROR,
        error: {
          error: response
        }
      });
  ...
  export default function* watcherSaga() {
    yield takeLatest(CommonCommunityActionTypes.COMMUNITY, fetchApiRequestSaga);
  }