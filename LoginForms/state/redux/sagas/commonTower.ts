...
const fetchApiData = (action: any) => NetworkService({
  ...
  function* fetchApiRequestSaga(action: any): any {
    ...
    yield put({ type: CommonTowerActionTypes.TOWER_LOADER, loading: true });
    const response : any = yield call(fetchApiData, action);

    if (response) {
      yield put({
        type: CommonTowerActionTypes.TOWER_SUCCESS,
        payload: {
          data: response.data
        },
      });
      ...
      yield put({
        type: CommonTowerActionTypes.TOWER_ERROR,
        error: {
          error: response
        }
      });
  ...
  export default function* watcherSaga() {
    yield takeLatest(CommonTowerActionTypes.TOWER, fetchApiRequestSaga);
  }