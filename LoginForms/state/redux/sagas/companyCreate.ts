...
const fetchApiData = (action: any) => NetworkService({
  ...
  function* fetchApiRequestSaga(action: any): any {
    ...
    yield put({ type: CompanyCreateActionTypes.COMPANY_CREATE_LOADER, loading: true });
    const response : any = yield call(fetchApiData, action);

    if (response) {
      yield put({
        type: CompanyCreateActionTypes.COMPANY_CREATE_SUCCESS,
        payload: {
          data: response?.data
        },
      });
      ...
      yield put({
        type: CompanyCreateActionTypes.COMPANY_CREATE_ERROR,
        error: {
          error: response
        }
      });
  ...
  export default function* watcherSaga() {
    yield takeLatest(CompanyCreateActionTypes.COMPANY_CREATE, fetchApiRequestSaga);
  }