...
function* fetchApiRequestSaga(action: any): any {
  yield put({
    type: SearchItemsActionTypes.SEARCH_ITEMS_LOADER,
    loading: true,
  });
  ...
  if (response) {
    yield put({
      type: SearchItemsActionTypes.SEARCH_ITEMS_SUCCESS,
      payload: {
        data: response.data,
      },
    });
    ...
    yield put({
      type: SearchItemsActionTypes.SEARCH_ITEMS_ERROR,
      error: {
        error: response,
      },
    });
...
export default function* watcherSaga() {
  yield takeLatest(SearchItemsActionTypes.SEARCH_ITEMS, fetchApiRequestSaga);
}
