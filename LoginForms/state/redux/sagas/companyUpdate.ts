...
const fetchApiData = (action: any) => NetworkService({
  ...
  function* fetchApiRequestSaga(action: any): any {
    ...
    yield put({ type: CompanyUpdateActionTypes.COMPANY_UPDATE_LOADER, loading: true });
    const response : any = yield call(fetchApiData, action);

    if (response) {
      yield put({
        type: CompanyUpdateActionTypes.COMPANY_UPDATE_SUCCESS,
        payload: {
          data: response?.data
        },
      });
      ...
      yield put({
        type: CompanyUpdateActionTypes.COMPANY_UPDATE_ERROR,
        error: {
          error: response
        }
      });
  ...
  export default function* watcherSaga() {
    yield takeLatest(CompanyUpdateActionTypes.COMPANY_UPDATE, fetchApiRequestSaga);
  }