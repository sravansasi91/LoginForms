...
const fetchApiData = (action: any) => NetworkService({
  ...
  function* fetchApiRequestSaga(action: any): any {
    ...
    yield put({ type: CommonUnitDetailActionTypes.UNIT_DETAIL_LOADER, loading: true });
    const response : any = yield call(fetchApiData, action);

    if (response) {
      yield put({
        type: CommonUnitDetailActionTypes.UNIT_DETAIL_SUCCESS,
        payload: {
          data: response.data
        },
      });
      ...
      yield put({
        type: CommonUnitDetailActionTypes.UNIT_DETAIL_ERROR,
        error: {
          error: response
        }
      });
  ...
  export default function* watcherSaga() {
    yield takeLatest(CommonUnitDetailActionTypes.UNIT_DETAIL, fetchApiRequestSaga);
  }