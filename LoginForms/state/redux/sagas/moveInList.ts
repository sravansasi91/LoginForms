...
const fetchApiData = (action: any) => NetworkService({
  ...
  function* fetchApiRequestSaga(action: any): any {
    ...
    yield put({ type: MoveInListActionTypes.MOVE_IN_LIST_LOADER, loading: true });
    const response : any = yield call(fetchApiData, action);

    if (response) {
      yield put({
        type: MoveInListActionTypes.MOVE_IN_LIST_SUCCESS,
        payload: {
          data: response.data
        },
      });
      ...
      yield put({
        type: MoveInListActionTypes.MOVE_IN_LIST_ERROR,
        error: {
          error: response
        }
      });
  ...
  export default function* watcherSaga() {
    yield takeLatest(MoveInListActionTypes.MOVE_IN_LIST, fetchApiRequestSaga);
  }