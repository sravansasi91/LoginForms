...
function* fetchApiRequestSaga(action: any): any {
  yield put({type: UserRoleSActionTypes.USER_ROLE_LOADER, loading: true});
  const response: any = yield call(fetchApiData, action);

  if (response) {
    yield put({
      type: UserRoleSActionTypes.USER_ROLE_SUCCESS,
      payload: {
        data: response.data,
      },
    });
    ...
    yield put({
      type: UserRoleSActionTypes.USER_ROLE_ERROR,
      error: {
        error: response,
      },
    });
...
export default function* watcherSaga() {
  yield takeLatest(UserRoleSActionTypes.USER_ROLE, fetchApiRequestSaga);
}
