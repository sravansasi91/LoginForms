...
const fetchApiData = (action: any) => NetworkService({
  ...
  function* fetchApiRequestSaga(action: any): any {
    yield put({ type: CountryActionTypes.COUNTRY_LOADER, loading: true });
    const response : any = yield call(fetchApiData, action);
    
    if (response) {
      yield put({
        type: CountryActionTypes.COUNTRY_SUCCESS,
        payload: {
          data: response?.data
        },
      });
      ...
      yield put({
        type: CountryActionTypes.COUNTRY_ERROR,
        error: {
          error: response
        }
      });
  ...
  export default function* watcherSaga() {
    yield takeLatest(CountryActionTypes.COUNTRY, fetchApiRequestSaga);
  }