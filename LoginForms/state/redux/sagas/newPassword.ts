...
const fetchApiData = (action: any) => NetworkService({
  ...
  function* fetchApiRequestSaga(action: any): any {
    yield put({ type: NewPasswordActionTypes.NEW_PASSWORD_LOADER, loading: true });
    const response : any = yield call(fetchApiData, action);
    if (response) {
      yield put({
        type: NewPasswordActionTypes.NEW_PASSWORD_SUCCESS,
        payload: {
          data: response?.data,
          email: action.body.data.email
        },
      ...
      yield put({
        type: NewPasswordActionTypes.NEW_PASSWORD_ERROR,
        error: {
          error: response
        }
      });
  ...
  export default function* watcherSaga() {
    yield takeLatest(NewPasswordActionTypes.NEW_PASSWORD, fetchApiRequestSaga);
  }