...
const fetchApiData = (action: any) => NetworkService({
  ...
  function* fetchApiRequestSaga(action: any): any {
    ...
    yield put({ type: OwnerCreateActionTypes.OWNER_CREATE_LOADER, loading: true });
    const response : any = yield call(fetchApiData, action);

    if (response) {
      yield put({
        type: OwnerCreateActionTypes.OWNER_CREATE_SUCCESS,
        payload: {
          data: response?.data
        },
      });
      ...
      yield put({
        type: OwnerCreateActionTypes.OWNER_CREATE_ERROR,
        error: {
          error: response
        }
      });
  ...
  export default function* watcherSaga() {
    yield takeLatest(OwnerCreateActionTypes.OWNER_CREATE, fetchApiRequestSaga);
  }