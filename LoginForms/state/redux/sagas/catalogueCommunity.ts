...
function* fetchApiRequestSaga(action: any): any {
  yield put({
    type: CatalogueCommunityActionTypes.CATALOGUE_COMMUNITY_LOADER,
    loading: true,
  });
  ...
  if (response) {
    yield put({
      type: CatalogueCommunityActionTypes.CATALOGUE_COMMUNITY_SUCCESS,
      payload: {
        data: response?.data,
      },
    });
    ...
    yield put({
      type: CatalogueCommunityActionTypes.CATALOGUE_COMMUNITY_ERROR,
      error: {
        error: response,
      },
    });
...
export default function* watcherSaga() {
  yield takeLatest(
    CatalogueCommunityActionTypes.CATALOGUE_COMMUNITY,
    fetchApiRequestSaga,
  );
}
